...
class Validation{
	...
	private function email($value){
		//$match = preg_match('/^\S+@\S+$/', $value);
		$match = preg_match('/^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/', $value);
		return $this->setError($match);
	}

	private function password($value){
		$match = preg_match('/^\S[a-zA-Z0-9_!@#$%&*()]*$/', $value);
		return $this->setError($match);
	}

	private function address($value){
		$match = preg_match('/^\d*\s[a-zA-Z\-\s\.]*$/i', $value);
		return $this->setError($match);
	}

	private function city($value){
		$match = preg_match('/^[a-zA-Z\s]+$/i', $value);
		return $this->setError($match);
	}
